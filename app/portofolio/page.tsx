...
function Detail() {
    ...
    return (
        <div className="px-6 pt-16 lg:px-24 lg:pt-24 mb-40">
            <section className="hero_section flex flex-col-reverse md:flex-row gap-4 ">
                <div className="md:w-1/3 flex flex-col justify-center">
                    <p className="title">{data?.project_title}</p>
                    <p className="muted mt-2">UI Design</p>
                    <div className="mt-6 mb:mt-16 ">
                        <a href={data?.ui_source} target="_blank" className="default-btn inline-flex items-center justify-center">
                ...
                <div className="flex space-x-6 border-b border-[#9AA6B2]">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`pb-2 text-md md:text-lg font-medium ${activeTab === tab
                                ? "border-b-2 border-[#4F98CA] dark:border-[#AD49E1] "
                ...
                <div className="flex flex-col md:flex-row mt-16 items-center md:justify-around">
                    {
                        data?.image_preview.map((item: string, i: number) => {
                            return (
                                <div key={i} className="mt-2 md:mt-10">
                                    <Image
                                        src={item}
                                        alt={'image'}
                                        width={200}
                                        height={500}
                                        className="w-screen h-auto md:h-[500px] md:w-[200px] object-contain"
                                    />
                                </div>
                            )
                        })
            ...
            <section id="feature" className="mt-24">
                <p className="section-title mb-6 mt-16">What We Offer</p>
                {
            ...
            <section id="stack" className="mt-24">
                <p className="section-title mb-6">What We Used :</p>
                <p >{data?.stack}</p>
            </section>
        </div >
    )
